...
export function LampComponent({ plancard }: { plancard: PlancardType[] }) {
  return (
    <LampContainer>
      <motion.h1
      ...
      <div className="max-w-[1440px] xl:px-[120px] md:px-[100px] sm:px-[50px]">
        <HoverEffect items={plancard} />
      </div>
    </LampContainer>
  );
}
...
export const LampContainer = ({
  ...
  return (
    <div
      className={cn(
        "relative flex flex-col items-center justify-center overflow-hidden w-full rounded-md",
        className
      )}
    >
      <div className="relative flex w-full flex-1 scale-y-125 items-center justify-center isolate top-[265px]">
        <motion.div
      ...
      <div className="relative top-[365px] flex -translate-y-80 flex-col items-center px-5 z-10">
        {children}
      </div>
    </div>
  );
